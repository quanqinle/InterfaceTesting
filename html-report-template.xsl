	...
	<html>
		<head>
			...
			<script language="JavaScript"><![CDATA[
				...
				{
					if (/\+/.test(document.getElementById(details_id+"_td").innerHTML))
					{
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<tr valign="top">
				...
				<td align="center" style="cursor:pointer">
					<xsl:attribute name="onclick"><xsl:text/>javascript:change('page_details_<xsl:value-of select="position()" />')</xsl:attribute>
					<xsl:attribute name="id"><xsl:text/>page_details_<xsl:value-of select="position()" />_td</xsl:attribute>+
				</td>
			</tr>
				...
				<td colspan="8" bgcolor="#FF0000">
					...
					<table bordercolor="#000000" bgcolor="#2674A6" border="0"  cellpadding="1" cellspacing="1" width="95%">
						...
						<tr>
							...
							<td align="center">
								<xsl:variable name="successVal" select="@s" />
								<xsl:if test="contains($successVal,'false')"><xsl:attribute name="class">Failure</xsl:attribute></xsl:if>
